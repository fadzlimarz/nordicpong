...
class Ball {
  ...
  color = null;
  cellColor = null;
  borderColor = null; // Add a property for the border color
  bColor = null;
  type = null;
  ...
  draw(ctx, stroke = false) {
    ...
    if (this.type === "norway") {
      this.drawNorwayFlag(ctx);
    } else {
      // Default drawing method
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
      ctx.fillStyle = this.color; // Primary color of the ball
      ctx.fill();

      // Draw the cross
      const crossWidth = this.size / 3; // Adjust the width of the cross as needed
      ctx.fillStyle = this.cellColor; // Color for the cross
  ...
  drawNorwayFlag(ctx) {
    // Main circle (red background)
    ctx.beginPath();
    ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
    ctx.fillStyle = this.color; // Red color
    ctx.fill();

    // Set widths for the cross and its border
    const crossWidth = this.size / 3;
    const borderSize = crossWidth / 4; // Adjusted white border size

    // White part of the cross (borders)
    ctx.fillStyle = this.bColor; // White color
    // Vertical part
    ctx.fillRect(
      this.x - crossWidth / 2 - borderSize,
      this.y - this.size,
      crossWidth + borderSize * 2,
      this.size * 2
    );
    // Horizontal part
    ctx.fillRect(
      this.x - this.size,
      this.y - crossWidth / 2 - borderSize,
      this.size * 2,
      crossWidth + borderSize * 2
    );

    // Blue part of the cross
    ctx.fillStyle = this.borderColor; // Blue color
    // Vertical part
    ctx.fillRect(
      this.x - crossWidth / 2,
      this.y - this.size,
